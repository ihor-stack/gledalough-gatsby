...
export const Seo = ({ description, title, image, children }) => {
  ...
  const ogImage = featuredImage && featuredImage?.publicURL
  ...
  const seo = {
    ...
    ogImage: image || `${site.siteMetadata.siteUrl}${ogImage}`,
    ogUrl: site.siteMetadata.siteUrl,
    ogImageSecureUrl: image || `${site.siteMetadata.siteUrl}${ogImage}`,
    twitterCard: site.twitterCard,
    twitterSite: site.twitterSite,
    twitterTitle: title || site.siteMetadata.title,
    twitterImage: image || `${site.siteMetadata.siteUrl}${ogImage}`,
  ...
  return (
    <Helmet>
      <title>Glendalough Distillery | {seo.title}</title>
      ...
      {seo.ogImage && (
        <meta
          ...
          content={image ? '1200' : seo.ogImage.width}
        />
      )}
      {seo.ogImage && (
        <meta
          property="og:image:height"
          content={image ? '630' : seo.ogImage.height}
        />
      )}
