...
const ThankYouWave = ({data}) => {
    // console.log('data ' , data)
    const pageContent = data.prismicThankYouWavePage
    const pageData = data.prismicThankYouWavePage.data
    ...
    return (
      <Layout currentPage="thank-you-wave" activeDocMeta={activeDoc}>
        <Seo
            title={pageData.meta_title?.text}
            description={ pageData.meta_description?.text}
            image={pageData.social_card?.url}
        />
        <section className="inside-content thank-you-wave">
          <CommonPageBanner
...
query ThankYouWavePageQuery($uid: String, $id: String, $lang: String){
    prismicThankYouWavePage(uid: { eq: $uid }, id: { eq: $id }, lang: { eq: $lang }) {
        _previewable
        url
        uid
        type
...
`
export default withPrismicPreview(ThankYouWave)